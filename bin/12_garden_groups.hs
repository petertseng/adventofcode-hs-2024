...
price2 s = length s * numSides s
  where numSides = sum . map numCorners
        set = Set.fromList s
        numCorners (y, x) = count id (us ++ ds)
          where us = if u then [l && not ul, r && not ur] else [not l, not r]
                ds = if d then [l && not dl, r && not dr] else [not l, not r]
                ul = (y - 1, x - 1) `Set.member` set
