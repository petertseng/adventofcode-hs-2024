...
data Directional = North | South | East | West | Activate deriving (Eq, Ord)

complexity :: Int -> Int -> Int -> Int -> Int
complexity n a b c = (a * 100 + b * 10 + c) * sum (Map.elems (iterate expandDir pairs0 !! n))
  where pairs0 = Map.fromListWith (+) (map (, 1) (pairs (Activate : expandNum [toEnum a, toEnum b, toEnum c, NA])))

expandDir :: Map (Directional, Directional) Int -> Map (Directional, Directional) Int
expandDir freq = Map.fromListWith (+) [((c, d), n) | ((a, b), n) <- Map.assocs freq, (c, d) <- pairs (Activate : dir a b ++ [Activate])]

expandNum :: [Numpad] -> [Directional]
expandNum = expand NA
  where expand _ [] = []
        expand cur (x:xs) = num cur x ++ Activate : expand x xs
