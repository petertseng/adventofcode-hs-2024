...
freqOfMode :: [Int] -> Int
freqOfMode xs = maximum (IntMap.elems freq)
  where freq = IntMap.fromListWith (+) (map (, 1) xs)
...
main = do
  ...
  let yt = maximumBy (comparing (freqOfMode . map snd . posesAt height width robots)) [0 .. height - 1]
      xt = maximumBy (comparing (freqOfMode . map fst . posesAt height width robots)) [0 .. width - 1]
